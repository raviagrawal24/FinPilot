...
export function KPICard({ metric }: KPICardProps) {
  ...
  return (
    <Card variant="glass" className="relative group hover:translate-y-[-2px] transition-all border-[var(--border-subtle)]">
      <div className="flex items-start justify-between">
        <div className="space-y-1">
          <p className="text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider">{metric.title}</p>
          <h3 className="text-2xl font-extrabold text-[var(--text-primary)] tracking-tight">{metric.formattedValue}</h3>
        </div>
        <div className="w-10 h-10 rounded-xl bg-[var(--bg-surface)] border border-[var(--border-subtle)] flex items-center justify-center group-hover:scale-110 transition-transform">
      ...
      <div className="mt-4 pt-3 border-t border-[var(--border-subtle)] flex items-center justify-between text-xs">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[var(--text-muted)] text-[11px]">vs last month</span>
        </div>
        {metric.subtitle && <span className="text-[11px] text-[var(--text-muted)] truncate max-w-[120px]">{metric.subtitle}</span>}
      </div>
    </Card>
  );
}
